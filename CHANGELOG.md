...
### ⚠️ BattleMetrics closed its free API

BattleMetrics now returns `403 Access denied. A subscription is required to use the API.` on every
endpoint the bot used — server details, player lists, profiles and leaderboards. Player tracking,
`/players` and the information-channel player list stopped working for everyone without a paid
BattleMetrics subscription.

This release adds a second player source that queries your Rust server directly, so tracking works
again without paying for BattleMetrics.

**What you need to do:** nothing, if your server's `connect` address was already stored — the bot
falls back automatically. Otherwise verify your server answers A2S queries and set `queryPort`:
...
- **A2S player source.** The bot can now read the player list straight from the Rust server using the
  Valve A2S query protocol, the same mechanism BattleMetrics and the in-game server browser use.
  Trackers, `/players`, the information channel and the WebUI player panel all work against it.
- New `playerSource` general setting: `auto` (default) prefers BattleMetrics and falls back to A2S
  when the API is unavailable, `battlemetrics` and `a2s` force a single source.
- New optional per-server `queryPort` (and `queryIp`) setting. The server list is keyed by the Rust+
  companion port (usually 28082), which is *not* the game port that answers A2S queries (usually
  28015), so the query port has to be configured or derived. The bot derives it from the stored
  `connect` string when possible and otherwise assumes the default Rust game port.
- `scripts/a2sProbe.js` for checking whether a server answers A2S queries with a usable player list
  before relying on it. It reports a clear verdict, including the case where a server reports players
  online but hides their names.
- Servers with no BattleMetrics id are now identified by a synthetic `a2s:host:port` id, which can
  also be typed into the BattleMetrics ID field of a server or tracker to attach an A2S source
  explicitly.
...
- Server name, status, address and streamer mode are resolved from the Rust+ connection and the
  stored server list when BattleMetrics is unavailable, instead of rendering as empty or "not found".
- The WebUI player panel now reports that the player source is unavailable instead of showing an
  empty list that was indistinguishable from "nobody is online".
- Player and server links are rendered as plain text for A2S sourced ids instead of pointing at
  BattleMetrics pages that do not exist.

### Known limitations of the A2S source

A2S reports player names and connected durations but no persistent player identifier, so compared to
BattleMetrics:

- A rename reads as a logout followed by a login rather than a name change.
- Two players using the same name collapse into a single entry.
- There is no player history, no profile links and no most-time-played leaderboard.
- Servers that hide their query player list cannot be tracked at all.
